...
/***************************************************************************************************************\
|*  Single segment producer-consumer using binary semaphores                                                   *|
|***************************************************************************************************************|
...
|*  This guarantees the execution sequence of W-R-W-R-...-W-R; it follows that                                 *|
...
create_shared_memory(shared_memory_t *self, const char *name, const char *write_sem_name, const char *read_sem_name)
{
    ...
    if (self->w_sem == SEM_FAILED) {
        dzlog_debug("sem_open() failed (@write_sem)");
        dzlog_debug("error: %d\n", errno);
    ...
    if (self->r_sem == SEM_FAILED) {
        dzlog_debug("sem_open() failed (@read_sem)");
        dzlog_debug("error: %d\n", errno);
...
write_shared_memory(shared_memory_t *self, void *data, size_t len, unsigned id, size_t total)
{
    ...
    dzlog_debug("This shared memory: r_sem =%d, w_sem = %d", rsemv, wsemv);
    dzlog_debug("write_shared_memory: waiting on semaphore...");
    sem_wait(self->w_sem);
    dzlog_debug("write_shared_memory: semaphore complete.");
    /* In process */
    ...
    /* (R,W) = (1,0) */
    dzlog_debug("write_shared_meory: posting semaphore");
    sem_post(self->r_sem);
    dzlog_debug("done.");
    return 0;
}
...
send_item(shared_memory_t *self, void *data, size_t len)
{
    ...
    for (i = 0; i < chunks; ++i) {
        ...
        dzlog_debug("send_item():  sending [%lu/%lu] chunks: start = %lu, len = %lu \n", i, chunks, ustart,
                    ulen);
        int r = write_shared_memory(self, data + ustart, ulen, (unsigned) i, chunks);
        if (r != 0) {
            dzlog_debug("send_item(): error sending [%lu/%lu] chunks: start = %lu, len = %lu \n", i, chunks, ustart,
                        ulen);
            return 1;
        }
    }
...
recv_item(shared_memory_t *self, void **data, size_t *len)
{
    ...
    void *temp;
    unsigned id;
    dzlog_debug("reading first chunk of memory...");
    ...
    dzlog_debug("memory size: %lu, total chunk: %lu", size_per_chunk, total_chunk);
    *data = malloc(total_chunk * size_per_chunk);
    ...
    for (i = 1; i < total_chunk - 1; ++i) {
        ...
        if (rt != 0) {
            free(*data);
            dzlog_debug("error reading chunk");
        }
    }
    if (total_chunk > 1) {
        void *_temp;
        ...
        if (rt != 0) {
            free(*data);
            dzlog_debug("error reading last chunk");
            return 1;
        }
